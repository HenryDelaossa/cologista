...
$("#logoutImg")
  .click(() => {
    ...
    setTimeout(() => {
      ...
      $(
        "#btnIngresar, #btnFlotInSes, #buttonRegisRightDark, .div__parrafo-a"
      ).fadeIn(400);
...
btnConfirmServ.addEventListener("click", (e) => {
  ...
  const arrInfoUltserv = [
    { dir1: infoinput1, dir2: infoinput2, valor: vbValr },
  ];
...
$.getJSON(EmpresAliadas, function (answer, status) {
  if (status === "success") {
    ...
    for (const LogoEmpresas of empresasAliadas) {
      $(".seccion-3-main-index__div-cont-divs-imgs")
        .append(`<div class=${LogoEmpresas.classCont}>
                    <img src=${LogoEmpresas.img} class=${LogoEmpresas.classImg} alt=${LogoEmpresas.alt}>
                </div>`);
    }
  }
});
// barra indicadora de altuta izquierda
indicatorLeft(".body");